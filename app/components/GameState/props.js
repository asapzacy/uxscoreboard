...
export const mlbGameStateProps = (game) => {
  ...
  const isDelayed = game.status.codedGameState === 'D' || game.status.statusCode === 'PR'
  ...
  return {
    ...
    time: isDelayed ? `${game.status.detailedState} - (${game.status.reason})` : `${formatTimezone(game.gameDate)} ET`,
    periods: 9,
