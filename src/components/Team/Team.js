...
import { team_colors as teamColors } from 'data/team_colors'
import s from './Team.scss'

const makeBgImg = (code, league, isLoaded) => ({
  backgroundImage: code !== 'nyy' && `linear-gradient(to right,${teamColors[league][code]} 40%,transparent 0%)`
})
...
const Team = ({ name, code, filetype = 'svg', ws, ls, ts, score, league, hasLoaded, logoHasLoaded }) => (
  <section className={code === 'nyy' ? s[code] : s.container} style={makeBgImg(code, league)}>
    <LogoContainer src={`/assets/public/img/${league}/teams/${code}.${filetype}`} name={name} hasLoaded={hasLoaded} logoHasLoaded={logoHasLoaded} />
