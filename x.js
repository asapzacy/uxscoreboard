...
{ gameState === 1 && <span>{`${currentTime} • ${currentPeriod}`}<sup>{inningSuffix(String(currentPeriod))}</sup></span> }
{ gameState === 2 && <span>{currentPeriod > periods ? `${status}/${ordinal}` : status}</span> }
